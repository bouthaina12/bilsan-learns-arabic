...
export function DashboardSidebar({ 
  ...
  return (
    <Sidebar 
      className={cn(
        "border-l border-pink-300 bg-gradient-to-b from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/10 transition-all duration-300 h-screen fixed right-0 top-0 shadow-lg",
        collapsed ? "w-16" : "w-64"
      )}
      collapsible="icon"
      style={{ zIndex: 40 }}
    >
      <SidebarHeader className="p-4 border-b border-sidebar-border">
        <Link to="" className="flex items-center gap-2">
          ...
          {!collapsed && (
            <span className="text-3xl font-bold text-primary font-arabic">
              بيلسان
            </span>
          )}
        </Link>
      ...
      {!collapsed && (
        <div className="p-4 border-b border-sidebar-border">
          <div className="text-center mb-4">
            ...
            <h3 className="font-arabic font-extrabold  text-foreground">{userName}</h3>
          </div>
                ...
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <Link to={item.url} className="flex items-center gap-3">
                      ...
                      {!collapsed && (
                        <span className="font-arabic font-bold">{item.title}</span>
                      )}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
